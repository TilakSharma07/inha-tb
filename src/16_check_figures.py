...
def main():
    figs, bad = [], []
    # data/fp_matrix.npy is 2.9 MB and regenerated by src/03, so it is not committed.
    # A fresh clone therefore cannot draw figures 1-2 until step 03 has run; say so
    # instead of dying in numpy.load with a bare FileNotFoundError.
    if os.path.exists(f"{D}/data/fp_matrix.npy"):
        ns = runpy.run_path(f"{SRC}/10_figures.py", run_name="not_main")
        figs += [("figure1", ns["figure1"]()), ("figure2", ns["figure2"]())]
    else:
        print("figure1, figure2 skipped: data/fp_matrix.npy absent "
              "(2.9 MB, not committed) - run `python src/03_features.py` first")
