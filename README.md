...
round-tripped matrix reproduces `data/metrics.csv` exactly - worst drift 0.000000 across
every metric, against up to 0.047 in MCC alone for a change of interpreter (table
above). So the boosted tree is stable under a 4.6e-13 perturbation of its inputs but
not across interpreters, which means the interpreter
